...
export default function HomePage() {
  ...
  const handleReset = () => { setLocation(''); setCourse(''); setMaxFees(''); setPage(1); };
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="flex gap-6">
          ...
          <div className="w-60 shrink-0 hidden md:block">
            <FilterPanel location={location} setLocation={setLocation}
              course={course} setCourse={setCourse}
              maxFees={maxFees} setMaxFees={setMaxFees} onReset={handleReset} />
          </div>
            ...
            {loading ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {Array(9).fill(0).map((_, i) => (
                  <div key={i} className="bg-[#111] rounded-2xl h-64 animate-pulse border border-[#1a1a1a]" />
                ))}
              </div>
              ...
              <div className="text-center py-20">
                ...
                <p className="text-[#555]">Try different search terms or filters</p>
              </div>
            ) : (
              <>
                <div className="flex justify-between items-center mb-5">
                  <p className="text-sm text-[#555]">
                    <span className="font-semibold text-white">{data?.total}</span> colleges found
                  </p>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                {data && data.totalPages > 1 && (
                  <div className="flex justify-center items-center gap-3 mt-8">
                    ...
                    <span className="text-sm text-[#555]">Page {page} of {data.totalPages}</span>
